...
def get_images_list(prints=False):
    ...
    if prints:
        ...
        print("-----------------------------------")
    urls = (
        [page.value for _, page in Pages.__members__.items()][1:]
        if collections[0] == Pages.ALL
        else [collection.value for collection in collections]
    )
    images = []
    for url in urls:
...
def getCollectionNames(collections=None):
    ...
    return ", ".join(
        [
            collection.name.lower().capitalize().replace("_", " ")
...
parser = argparse.ArgumentParser(
    ...
    prog="stalenhag",
    epilog="Cheers to Simon for making his awesome art available!",
)
parser.add_argument(
    "-a", "--all", help="Download all images to local directory", action="store_true"
)
...
parser.add_argument(
    "-s", "--save", help="Save current wallpaper to favorites", action="store_true"
)
...
parser.add_argument(
    "--fav",
    "--favorite",
    help="Set random wallpaper from favorites",
    action="store_true",
)
...
def run():
    ...
    if args.all:
        ...
        error = len(collections) == 0 or not all(
            [collection in collection_names for collection in collections]
        )
        if error:
